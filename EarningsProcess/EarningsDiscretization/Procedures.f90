...
SUBROUTINE SymmetricPowerSpacedGrid (n,k,width,center,y)

!gives a grid spaced between center-width and center+width based on the interval [-1,1] with a function x^(1/k) either side
!k = 1 is linear, k = 0 is L-shaped

IMPLICIT NONE

INTEGER, INTENT(in)	:: n
REAL(8), INTENT(in)	:: k,width,center
REAL(8), INTENT(out) :: y(:)
INTEGER				:: i
REAL(8)				:: x(n),z(n)
...
CALL LinearSpacedGrid (n,-1.0_8,1.0_8,x)

DO i = 1,n
	IF(x(i)>0.0) z(i) = x(i)**(1.0/k)
	IF(x(i)==0.0) z(i) = 0.0
	IF(x(i)<0.0) z(i) = -((-x(i))**(1.0/k))
END DO
...
END SUBROUTINE SymmetricPowerSpacedGrid
...
!this takes in xi, and finds two points in either side of xi in x
